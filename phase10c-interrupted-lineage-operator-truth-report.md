1. Concise diagnosis
- Tuku could preserve interrupted lineage truthfully, but there was no first-class operator action to acknowledge that lineage as reviewed.
- That left interrupted recovery as a passive posture with no dedicated CLI -> IPC -> daemon -> orchestrator loop.
- Operator surfaces could explain interrupted recovery, but they could not durably record that the interrupted lineage had been intentionally reviewed.

2. Exact implementation plan executed
- Added a new typed recovery action kind: `INTERRUPTED_RUN_REVIEWED`.
- Added a dedicated proof event: `INTERRUPTED_RUN_REVIEWED`.
- Extended `RecordRecoveryAction` to accept interrupted-lineage review only when recovery class is `INTERRUPTED_RUN_RECOVERABLE`.
- Kept interrupted review semantically narrow: it does not change recovery class, does not enable fresh run start, and keeps recommended action as `RESUME_INTERRUPTED_RUN`.
- Added a dedicated CLI / IPC / daemon route: `tuku recovery review-interrupted --task <id> [--summary ...] [--note ...]` -> `task.recovery.review_interrupted` -> daemon -> existing orchestrator `RecordRecoveryAction` logic.
- Updated shell proof summaries so interrupted review is legible in shell snapshot/operator surfaces.
- Added focused orchestrator, daemon, app, and shell regressions for posture, proof, replay, and blocked run-start behavior.
...
- Before: interrupted recovery could be identified, but not durably reviewed through a dedicated operator command.
- After: interrupted lineage can be explicitly reviewed and recorded through a real CLI/IPC/daemon/orchestrator path.
- Before: there was only generic `recovery record` for backend use.
- After: there is a dedicated operator verb for interrupted-lineage handling.
- Before: shell proof highlights had no interrupted-review-specific summary.
- After: interrupted review appears as a distinct proof highlight.
- Before: interrupted review was not a first-class durable operator step.
- After: it is durable, inspectable, replay-safe, and still non-startable.

5. Interrupted-lineage semantics after this phase
- `INTERRUPTED_RUN_RECOVERABLE` still means Tuku has preserved an interrupted execution lineage as recoverable.
- `INTERRUPTED_RUN_REVIEWED` means the operator explicitly reviewed that interrupted lineage.
- After review:
  - recovery class remains `INTERRUPTED_RUN_RECOVERABLE`
  - recommended action remains `RESUME_INTERRUPTED_RUN`
  - `ReadyForNextRun` remains `false`
  - fresh `RunTask(start)` remains blocked
- Tuku still does not claim runtime resume exists in v1.
- The new operator loop closes the acknowledgment gap without pretending PTY/process/session continuation exists.

6. Tests added or updated
- Added orchestrator test covering valid interrupted review, proof emission, status/inspect/shell alignment, and blocked run start after review.
- Added orchestrator test covering invalid-posture rejection and idempotent replay for interrupted review.
- Added shell snapshot regression showing interrupted review remains interrupted-recoverable and emits a dedicated proof summary.
- Added daemon route test for `task.recovery.review_interrupted`.
- Added CLI route test and daemon-error propagation test for `tuku recovery review-interrupted`.
...
- Interrupted lineage review is still an operator acknowledgment, not an executable runtime resume workflow.
- There is still no dedicated `resume interrupted run` execution mutation in v1.
- Shell snapshot surfaces the interrupted review via recovery reason and proof highlights, not via a dedicated shell-level latest-recovery-action field.
- Recovery/readiness remains projected on read rather than persisted as a consolidated control-plane state object.
...
func (a *CLIApplication) Run(ctx context.Context, args []string) error {
	...
	case "recovery":
		if len(args) < 2 {
			return errors.New("usage: tuku recovery <record|review-interrupted|rebrief|continue> ...")
		}
		switch args[1] {
		case "record":
