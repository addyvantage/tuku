...
- Tuku could durably record `DECISION_CONTINUE`, but that only selected a recovery branch. It did not execute a real operator-side continuation workflow.
- The continue branch was therefore weaker than the rebrief branch: rebrief had an explicit executable mutation, while continue only changed projected posture.
- There was no durable proof-backed artifact showing that the operator had explicitly finalized continuation with the current brief.
- There was no dedicated CLI/IPC/daemon path for executing that continuation step.

2. Exact implementation plan executed
- Added a dedicated orchestrator mutation `ExecuteContinueRecovery` that finalizes the continue branch without starting a run.
- Restricted execution to the conservative posture: current recovery class must be `READY_NEXT_RUN` and latest recovery action must be `DECISION_CONTINUE`.
- Persisted a durable `CONTINUE_EXECUTED` recovery action so status/inspect can distinguish decision selection from decision execution.
- Added a crisp proof event `RECOVERY_CONTINUE_EXECUTED` and canonical response emission.
- Kept the current brief canonical and unchanged; updated capsule phase/next-action to explicitly reflect operator-cleared readiness for the next bounded run.
- Added `tuku recovery continue --task <id>` and routed it through IPC and daemon to the orchestrator mutation.
- Rejected replay after success once latest recovery action is already `CONTINUE_EXECUTED`, avoiding churn.
- Updated recovery projection so explicit continue execution is reflected in the recovery reason while preserving the same ready-next-run class/action.
...
- Before: `DECISION_CONTINUE` made the task look ready, but there was no explicit durable execution step proving the operator had finalized that branch.
- After: `ExecuteContinueRecovery` durably records that operator confirmation, emits proof, and keeps the current brief as the canonical brief for the next bounded run.
- Before: there was no command surface for continuing recovery.
- After: operators can run `tuku recovery continue --task <id>` through the real CLI -> IPC -> daemon -> orchestrator path.
- Before: replaying the continue branch after success had no explicit execution semantics.
- After: repeat execution is rejected once the branch has already been finalized.

5. New continue-execution recovery semantics introduced
- New orchestrator mutation:
  - `ExecuteContinueRecovery(ctx, ExecuteContinueRecoveryRequest)`
- New durable recovery action kind used as the explicit execution artifact:
  - `CONTINUE_EXECUTED`
- New proof event:
  - `RECOVERY_CONTINUE_EXECUTED`
- New CLI command:
  - `tuku recovery continue --task <id>`
- New IPC method:
  - `task.recovery.continue`
- Execution preconditions:
  - recovery class is `READY_NEXT_RUN`
  - latest durable recovery action is `DECISION_CONTINUE`
- Execution effects:
  - current brief remains canonical
  - capsule stays in `BRIEF_READY`
  - capsule `NextAction` is updated to explicit run-cleared wording
  - `CONTINUE_EXECUTED` recovery action is persisted
  - `RECOVERY_CONTINUE_EXECUTED` proof event is appended
  - canonical response is emitted
- Replay rule:
  - if latest recovery action is already `CONTINUE_EXECUTED`, the workflow is rejected rather than re-applied

6. Tests added or updated
- Orchestrator:
  - successful continue-execution from valid posture
  - invalid-posture rejection
  - replay-safe repeated execution rejection
  - current brief remains canonical
  - proof event emission
  - status/inspect reflect explicit continue execution
- Daemon:
  - `task.recovery.continue` request/response route mapping
- App CLI:
  - `tuku recovery continue --task <id>` request routing
  - daemon rejection surfacing
- Shell-facing:
  - proof summary mapping for `RECOVERY_CONTINUE_EXECUTED`

7. Commands run
```bash
gofmt -w internal/domain/proof/types.go internal/orchestrator/orchestrator.go internal/orchestrator/continue_execution.go internal/orchestrator/recovery.go internal/orchestrator/shell.go internal/ipc/types.go internal/ipc/payloads.go internal/runtime/daemon/service.go internal/runtime/daemon/service_test.go internal/app/bootstrap.go internal/app/bootstrap_test.go internal/orchestrator/service_test.go
...
- Continue execution now finalizes the recovery branch, but it still does not start a run. That remains a separate explicit operator step.
- The workflow is intentionally narrow: it confirms the current brief only; it does not revalidate or regenerate intent/context.
- Replay is rejected after success rather than reused. That is the safer choice for now, but it means operators must inspect status rather than expect no-op reuse.
- Recovery posture is still projected on read; there is no broader consolidated workflow-state object.
...
func (a *CLIApplication) Run(ctx context.Context, args []string) error {
	...
	case "recovery":
		if len(args) < 2 {
			return errors.New("usage: tuku recovery <record|rebrief|continue> ...")
		}
		switch args[1] {
		case "record":
...
func TestCLIRecoveryContinueCommandReturnsDaemonError(t *testing.T) {
	...
	ipcCall = func(_ context.Context, _ string, req ipc.Request) (ipc.Response, error) {
		return ipc.Response{}, errors.New("daemon error [CONTINUE_RECOVERY_FAILED]: continue recovery can only be executed while recovery class is READY_NEXT_RUN and latest action is DECISION_CONTINUE")
	}

	app := NewCLIApplication()
	err := app.Run(context.Background(), []string{"recovery", "continue", "--task", "tsk_789"})
	if err == nil || !strings.Contains(err.Error(), "READY_NEXT_RUN") {
...
func TestExecuteContinueRecoveryRejectsInvalidPosture(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), string(RecoveryClassReadyNextRun)) || !strings.Contains(err.Error(), string(recoveryaction.KindDecisionContinue)) {
		t.Fatalf("expected ready-next-run/decision-continue rejection, got %v", err)
	}
}
