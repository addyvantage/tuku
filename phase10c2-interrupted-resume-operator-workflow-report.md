1. Concise diagnosis
- Tuku could detect, reconcile, and review interrupted lineage, but there was still no dedicated operator execution step for `RESUME_INTERRUPTED_RUN`.
- That left interrupted lineage as a recommendation rather than a completed operator loop.
- The control plane could say “resume interrupted run,” but it could not durably move that lineage into the next explicit control-plane posture without pretending runtime resume existed.

2. Exact implementation plan executed
- Added a new recovery action kind: `INTERRUPTED_RESUME_EXECUTED`.
- Added a dedicated proof event: `INTERRUPTED_RUN_RESUME_EXECUTED`.
- Added a dedicated orchestrator mutation: `ExecuteInterruptedResume`.
- Restricted it to the honest current posture:
  - `RecoveryClassInterruptedRunRecoverable`
  - `RecoveryActionResumeInterrupted`
- Implemented the preferred transition target:
  - `RecoveryClassContinueExecutionRequired`
  - `RecoveryActionExecuteContinueRecovery`
  - `ReadyForNextRun = false`
- Updated recovery progression so the new action durably projects that next posture.
- Added CLI / IPC / daemon route:
  - `tuku recovery resume-interrupted --task <id> [--summary ...] [--note ...]`
  - `task.recovery.resume_interrupted`
- Updated `ExecuteContinueRecovery` so interrupted-resume is a valid trigger into the existing continue-finalization step.
- Added focused orchestrator, daemon, app, and shell regressions.
...
- `/Users/kagaya/Desktop/Tuku/internal/orchestrator/interrupted_resume.go`
...
- Before: interrupted lineage could be identified and reviewed, but not explicitly resumed through a dedicated operator workflow.
- After: operators can execute `tuku recovery resume-interrupted` as a real control-plane mutation.
- Before: `RESUME_INTERRUPTED_RUN` was only a posture/recommendation.
- After: it becomes a durable operator step that transitions into `CONTINUE_EXECUTION_REQUIRED`.
- Before: there was no dedicated proof artifact for interrupted-lineage continuation.
- After: Tuku records `INTERRUPTED_RUN_RESUME_EXECUTED`.
- Before: interrupted lineage continuation had no dedicated CLI/IPC/daemon path.
- After: the end-to-end command path exists and remains honest.

5. Resulting interrupted-resume semantics
- `INTERRUPTED_RUN_RECOVERABLE` still means interrupted lineage is preserved and recoverable, not fresh-run ready.
- `ExecuteInterruptedResume` means the operator explicitly advances interrupted lineage as a control-plane step.
- After interrupted resume:
  - recovery becomes `CONTINUE_EXECUTION_REQUIRED`
  - recommended action becomes `EXECUTE_CONTINUE_RECOVERY`
  - `ReadyForNextRun` remains `false`
  - fresh `RunTask(start)` remains blocked
- This does not:
  - start a worker
  - reattach a session
  - replay a transcript
  - claim runtime resume happened
- The next explicit step remains `ExecuteContinueRecovery`.

6. Tests added or updated
- Added orchestrator happy-path test for interrupted resume.
- Added orchestrator test proving fresh run start stays blocked after interrupted resume.
- Added orchestrator invalid-posture + replay rejection test.
- Added orchestrator test proving `ExecuteContinueRecovery` accepts interrupted-resume as a valid trigger.
- Added shell snapshot regression showing interrupted resume becomes `CONTINUE_EXECUTION_REQUIRED` while staying not fresh-run ready.
- Added daemon route test for `task.recovery.resume_interrupted`.
- Added CLI route test and daemon-error propagation test for `tuku recovery resume-interrupted`.
- Updated continue-recovery invalid-posture expectation to include the new valid trigger kind.

7. Commands run
```bash
gofmt -w internal/domain/recoveryaction/types.go internal/domain/proof/types.go internal/orchestrator/orchestrator.go internal/orchestrator/recovery.go internal/orchestrator/shell.go internal/ipc/types.go internal/ipc/payloads.go internal/orchestrator/interrupted_resume.go internal/runtime/daemon/service.go internal/app/bootstrap.go internal/runtime/daemon/service_test.go internal/app/bootstrap_test.go internal/orchestrator/service_test.go internal/orchestrator/continue_execution.go internal/orchestrator/shell_test.go
...
- This is still not runtime resume. It is a control-plane lineage continuation step only.
- Interrupted lineage still requires `ExecuteContinueRecovery` before a fresh bounded run can ever become eligible.
- There is still no executable PTY/process/session continuation in v1.
- Recovery and readiness are still projected on read rather than persisted as a consolidated control-plane state object.
